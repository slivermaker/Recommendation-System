...
*/
    val hdfsPath = "hdfs://localhost:9000/best_model"

    val loadedModel = ALSModel.load(hdfsPath)
    var bestModel_run = loadedModel
    // 用最佳模型预测测试集的评分,并计算和实际评分之间的均方根误差
    val testRmse = computeRmse(bestModel_run, testDF, numTest)
    ...
    val recommendations = bestModel_run
